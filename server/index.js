...
async function run() {
  try {
    ...
    // all user collection load in client side
    ...
    app.patch("/user-request/:email", verifyToken, async (req, res) => {
      ...
      if (!user || user?.status === "Requested")
        return res.status(400).send("requested already approved!!");
      const updateDock = {
        $set: {
          status: "Requested",
        },
    ...
    // user collection to customer request seller account admin approved
    app.patch("/update-seller/:email", verifyToken, async (req, res) => {
      const email = req.params.email;
      ...
      const result = await userCollection.updateOne(filter, updateDock);
      res.send(result);
    });
    ...
    // delete plants
    app.delete("/delete-plants/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
    ...
    app.put("/update-plant/:id", async (req, res) => {
      ...
      const options = { upsert: false }; // replace only if exists
      const result = await plantCollection.updateOne(filter, {
        $set: updatedUser,
      } , options);
      res.send(result);
    });
    ...
    app.get("/customer-plant/:email", verifyToken, async (req, res) => {
      ...
      const result = await orderCollection
        .aggregate([
          ...
          {
            $addFields: {
              //plant id will just id so we will just id to convert object id
              plantId: { $toObjectId: "$plantId" },
            },
          },
          {
            $lookup: {
              //collection match and before id and now id match to return plants
              from: "plants",
              localField: "plantId",
              foreignField: "_id",
              as: "plants",
          ...
          {
            $addFields: {
              //plant object to pacific data get
              name: "$plants.name",
              image: "$plants.image",
          ...
          {
            $project: {
              //I will plants object block or remove
              plants: 0,
            },
          },
        ])
    ...
    // seller can customer order cancel
    app.get("/seller-plant/:email", verifyToken, async (req, res) => {
      const email = req.params.email;
      const query = { "customer.email": email };
      const result = await orderCollection
        .aggregate([
